...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white dark:bg-surface-950">
      {/* Navbar */}
      <header className="sticky top-0 z-40 border-b border-surface-200/80 bg-white/90 backdrop-blur-xl dark:border-surface-700/80 dark:bg-surface-950/90">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            {['Fonctionnalités', 'Tarifs', 'À propos', 'Blogue'].map((item) => (
              <a key={item} href="#" className="text-sm text-surface-600 hover:text-surface-900 dark:text-surface-400 dark:hover:text-surface-50">
                {item}
              </a>
            ))}
          </nav>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden bg-white px-4 py-20 sm:px-6 lg:px-8 lg:py-28 dark:bg-surface-950">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -right-40 -top-40 h-[500px] w-[500px] rounded-full bg-orange-50 dark:bg-orange-900/20" />
          <div className="absolute -bottom-20 -left-20 h-[300px] w-[300px] rounded-full bg-orange-50/60 dark:bg-orange-900/10" />
        </div>
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="grid items-center gap-12 lg:grid-cols-2">
            {/* Left */}
            <div>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-1.5 text-sm font-medium text-orange-600 dark:bg-orange-900/30 dark:text-orange-400">
                <Star className="h-3.5 w-3.5 fill-orange-500 text-orange-500" />
                Noté 4.9/5 par 10 000+ restaurants
              </div>

              <h1 className="text-4xl font-bold leading-tight text-gray-900 sm:text-5xl lg:text-6xl dark:text-surface-50">
                Obtenez vos repas
                <span className="block text-brand-500">favoris livrés</span>
                partout, toujours
              </h1>

              <p className="mt-5 text-lg text-gray-500 leading-relaxed dark:text-surface-400">
              ...
              <div className="mt-8 flex items-center gap-6">
                <div className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-surface-400">
            ...
            <div className="relative hidden lg:flex items-center justify-center">
              <div className="relative rounded-3xl bg-gradient-to-br from-orange-500 to-orange-400 p-8 shadow-2xl shadow-orange-200">
                ...
                <div className="mt-4 rounded-2xl bg-white p-4 dark:bg-surface-900">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-orange-100 text-2xl dark:bg-orange-900/30">🍔</div>
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-gray-900 dark:text-surface-50">Classic Smash Burger</p>
                      <p className="text-xs text-gray-400 dark:text-surface-500">Le Comptoir Moderne · 12 min</p>
                    </div>
              ...
              {/* Floating badge */}
              <div className="absolute -bottom-4 -left-4 rounded-2xl bg-white px-4 py-3 shadow-lg dark:bg-surface-900 dark:border dark:border-surface-700">
                <p className="text-xs text-gray-500 dark:text-surface-400">Commandes aujourd&apos;hui</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-surface-50">2 847</p>
      ...
      {/* Stats */}
      <section className="border-y border-surface-200 bg-surface-50 py-12 dark:border-surface-700 dark:bg-surface-900">
      ...
      {/* Features */}
      <section className="py-24 px-4 sm:px-6 lg:px-8 dark:bg-surface-950">
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            <h2 className="text-3xl font-bold text-surface-900 sm:text-4xl dark:text-surface-50">
          ...
          <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((feature) => (
              <div
                key={feature.title}
                className="group rounded-2xl border border-surface-200 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-md dark:border-surface-700 dark:bg-surface-900"
              >
                <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-2xl ${feature.bg}`}>
                  <feature.icon className={`h-6 w-6 ${feature.color}`} />
                </div>
                <h3 className="mb-2 text-lg font-semibold text-surface-900 dark:text-surface-50">{feature.title}</h3>
                <p className="text-sm text-surface-500 leading-relaxed dark:text-surface-400">{feature.description}</p>
      ...
      {/* Testimonials */}
      <section className="bg-surface-50 py-24 px-4 sm:px-6 lg:px-8 dark:bg-surface-900">
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            <h2 className="text-3xl font-bold text-surface-900 sm:text-4xl dark:text-surface-50">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-3">
            {testimonials.map((t) => (
              <div key={t.name} className="rounded-2xl border border-surface-200 bg-white p-6 shadow-sm dark:border-surface-700 dark:bg-surface-800">
                <div className="mb-4 flex gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className="h-4 w-4 fill-brand-400 text-brand-400" />
                  ))}
                </div>
                <p className="text-sm text-surface-600 leading-relaxed dark:text-surface-400">&ldquo;{t.text}&rdquo;</p>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 dark:bg-surface-950">
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg text-surface-500 dark:text-surface-400">Pas de surprises. Changez de plan à tout moment.</p>
          </div>
          <div className="mt-12 grid gap-6 sm:grid-cols-3">
              ...
              <div
                ...
                className={`relative rounded-2xl border p-8 ${
                  plan.highlighted
                    ...
                    : 'border-surface-200 bg-white shadow-sm dark:border-surface-700 dark:bg-surface-900'
                }`}
              >
                {plan.highlighted && (
                ...
                <h3 className={`text-xl font-bold ${plan.highlighted ? 'text-white' : 'text-surface-900 dark:text-surface-50'}`}>
                  {plan.name}
                </h3>
                <p className={`mt-1 text-sm ${plan.highlighted ? 'text-white/80' : 'text-surface-500 dark:text-surface-400'}`}>
                  {plan.description}
                </p>
                <div className="mt-4">
                  {plan.price === 'Sur mesure' ? (
                    <span className={`text-3xl font-bold ${plan.highlighted ? 'text-white' : 'text-surface-900 dark:text-surface-50'}`}>
                      Sur mesure
                    </span>
                  ) : (
                    <div className="flex items-baseline gap-1">
                      <span className={`text-4xl font-bold ${plan.highlighted ? 'text-white' : 'text-surface-900 dark:text-surface-50'}`}>
                        {plan.price}€
                      </span>
                      <span className={`text-sm ${plan.highlighted ? 'text-white/70' : 'text-surface-400 dark:text-surface-500'}`}>/mois</span>
                    </div>
                  )}
                </div>
                <ul className="mt-6 space-y-3">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2.5">
                      ...
                      />
                      <span className={`text-sm ${plan.highlighted ? 'text-white/90' : 'text-surface-600 dark:text-surface-400'}`}>
                        {feature}
                      </span>
                    </li>
                  ))}
      ...
      {/* Footer */}
      <footer className="border-t border-surface-200 bg-white py-12 px-4 sm:px-6 lg:px-8 dark:border-surface-700 dark:bg-surface-950">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-surface-900 dark:text-surface-50">FoodStack</span>
            </div>
            ...
            <div className="flex gap-4">
              {['Confidentialité', 'CGU', 'Contact'].map((item) => (
                <a key={item} href="#" className="text-sm text-surface-400 hover:text-surface-700 dark:text-surface-500 dark:hover:text-surface-300">
                  {item}
                </a>
              ))}
            </div>
