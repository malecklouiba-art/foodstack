...
const INIT_KIOSKS: Kiosk[] = [
  { id: 'k1', name: 'Borne #1 — Entrée', location: 'Hall principal', active: true,  mode: 'both',      ordersToday: 34, lastPing: 'il y a 10s',  color: '#1EFF6A', waitTime: 12 },
  { id: 'k2', name: 'Borne #2 — Salle',  location: 'Salle principale', active: true,  mode: 'sur_place', ordersToday: 22, lastPing: 'il y a 25s',  color: '#1EFF6A', waitTime: 15 },
...
function AddKioskModal({ onClose, onAdd }: { onClose: () => void; onAdd: (k: Kiosk) => void }) {
  const [form, setForm] = useState({ name: '', location: '', mode: 'both' as KioskMode, color: '#1EFF6A' });
